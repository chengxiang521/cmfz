    ...
    <script type="text/javascript">
        ...
        $(function () {
            $.ajax({
                type: 'POST',
                dataType: "json",
                url: '${pageContext.request.contextPath}/menu/all',//获取菜单
                success: function (data) {
                    $.each(data, function (i, result) {
                        var strcon = "";
                        if (result.cnode != null) {
                            $.each(result.cnode, function (index, row) {
                                strcon += "<a id=\"btn\" href=\"#\" class=\"easyui-linkbutton\" onclick=\"addTabs('" + row.title + "','" + row.url +
                                    "')\" data-options=\"iconCls:'icon-search'\">" + row.title + "</a><br>";
                            });
                        }
                        $('#aa').accordion('add', {
                            title: result.title,
