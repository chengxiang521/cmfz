	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#captchaImage").click(function(){//点击更换验证码
				$("#captchaImage").prop("src","${pageContext.request.contextPath }/code/get?"+new Date());
			});
			//失去焦点自动验证
			$("#enCode").blur(function(){
				...
				$.ajax({
						type:"POST",
						url:"${pageContext.request.contextPath }/code/validate",
						data:"code="+code,
						dataType:"json",
						success:function (result) {
							if(result==="Y"){
								$('#loginf1').html("正确").css({"color":"green"});
								flg=true;
							}else {
								$('#loginf1').html("错误").css({"color":"red"});
								flg= false;
							}
						}
						});
			...
			//  form 表单提交
			 $("#loginForm").bind("submit",function(){
			 	if(flg===false){return false;}else {return true;}
			 });
		});
	</script>
</head>
					  ...
					  <tr>
						...
						<tr>
							...
							<td>
								...
								<font id="loginf1"> </font>
								<img id="captchaImage" class="captchaImage" src="${pageContext.request.contextPath }/code/get" title="点击更换验证码"/>
							</td>
						</tr>					
					<tr>
						<td>
